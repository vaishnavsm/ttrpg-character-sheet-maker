...
const entry = z.strictObject({ name: label, description: z.string().max(8000).default("") });
...
export const characterSchema = z.strictObject({
  ...
  resources: z.array(z.strictObject({ name: label, maximum: z.number().int().min(1).max(30), used: z.number().int().min(0).default(0) }).refine(r => r.used <= r.maximum, { message: "Used cannot exceed maximum", path: ["used"] })).max(30).default([]),
  ...
  personality: z.array(z.strictObject({ label, text: z.string().max(8000) })).max(20).default([]),
  spellcasting: z.strictObject({
    ability: label,
    saveDC: value,
    attackBonus: value,
    slots: z.array(z.strictObject({ level: label, total: z.number().int().min(1).max(20) })).max(12).default([]),
    spells: z.array(entry.extend({ level: z.string().max(40) })).max(150).default([]),
  }).optional(),
  sections: z.array(z.strictObject({ title: label, column: z.enum(["left", "middle", "right"]).default("right"), entries: z.array(entry).max(100) })).max(30).default([]),
